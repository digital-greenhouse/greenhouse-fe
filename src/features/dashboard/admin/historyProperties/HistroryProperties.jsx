...
function resolvePropertyStatusKey(property) {
...
function HistoryProperties() {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            ...
            try {
                ...
                setProperties([]);
                setFeedback('No se pudo cargar el listado de propiedades.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const filteredProperties = useMemo(() => {
        ...
        const selectedState = properties.filter((record) => {
            const status = resolvePropertyStatusKey(record) || '';
    ...
    }, [properties, searchQuery, selectedStatus, selectedDateMode, selectedDate]);
    ...
    const columns = useMemo(() => ([
        ...
        {
            name: 'Capacidad',
            selector: (row) => row?.max_capacity ?? 'No disponible',
            sortable: true,
            grow: 0.6,
        },
        {
            name: 'Precio base',
            selector: (row) => `$${Number(row?.base_price_per_night || 0).toLocaleString('es-CO')}`,
            sortable: true,
            grow: 0.8,
        },
        {
            name: 'Estado',
            selector: (row) => statusLabels[resolvePropertyStatusKey(row)] || resolvePropertyStatusKey(row) || 'No disponible',
            sortable: true,
            grow: 0.7,
        },
        {
    ...
    return (
        <section className="history-properties">
            <header className="history-properties__header">
                <p className="history-properties__kicker">Historial</p>
                <h2 className="history-properties__title">Propiedades registradas</h2>
            </header>
            ...
            <DataTable
                ...
                progressComponent={(
                    <div className="history-properties__loading">
                        <Spinner animation="border" size="lg" />
                    </div>
                )}
                noDataComponent={(
                    <div className="history-properties__empty">
                        {feedback || 'No hay propiedades para mostrar.'}
                    </div>
                )}
            />
        </section>
